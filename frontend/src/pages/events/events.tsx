...
const themeValues = ["SPORT", "LANGUES", "ETUDE", "METIERS", "LOISIR"];
...
export default function EventPage(): JSX.Element {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <NavBar />
      <div className="flex flex-col flex-grow p-5 w-dvw mt-20">
        ...
        <div className="flex flex-col space-y-8 w-full justify-center items-center mt-10">
          ...
          {showForm && (
            <form
              className="w-5/6 p-5 border border-1 rounded-lg"
                ...
                <div className="flex space-x-4">
                  <label className="flex items-center space-x-1">
                    <input
                      ...
                      onChange={() =>
                        setNewEvent((prev) => ({ ...prev, isOnline: true }))
                      }
                    />
                    <span>Oui</span>
                  </label>
                  ...
                  <label className="flex items-center space-x-1">
                    <input
                      ...
                      onChange={() =>
                        setNewEvent((prev) => ({ ...prev, isOnline: false }))
                      }
                    />
                    <span>Non</span>
                  </label>
                ...
                <div className="mb-4">
                    ...
                    <ul className="absolute z-10 bg-white border rounded w-full mt-1 max-h-48 overflow-y-auto">
                      {filteredAdresses.length > 0 ? (
                        ...
                        <li className="px-3 py-2 text-gray-600">
                          Pas d'adresse correspondant à la recherche, veuillez
                          en créer une.
                        </li>
                      )}
                    </ul>
                  ...
                  {newEvent.adresse && (
                    <div className="mt-2">
                      ...
                      {newEvent.adresse.numero} {newEvent.adresse.rue},{" "}
                      {newEvent.adresse.codePostal} {newEvent.adresse.ville}
                    </div>
                  )}
                </div>
              )}
          ...
          {hoveredEvent && (
            <div
              className="fixed bg-white border p-4 rounded shadow-lg z-50"
              style={{ top: "10px", left: "10px" }}
            >
              <h2 className="font-bold pb-2" style={{ fontSize: "1.1rem" }}>
                {hoveredEvent.nom}
              </h2>
              <p>
                <strong>Thème:</strong> {hoveredEvent.theme}
              </p>
              <p>
                <strong>Date de début:</strong>{" "}
                {new Date(hoveredEvent.dateDebut).toLocaleDateString()}
              </p>
              <p>
                <strong>Date de fin:</strong>{" "}
                {new Date(hoveredEvent.dateFin).toLocaleDateString()}
              </p>
              <p>
                <strong>Prix Normal:</strong> {hoveredEvent.prixNormal} €
              </p>
              <p>
                <strong>Prix Adhérent:</strong> {hoveredEvent.prixAdherent} €
              </p>
              <p>
              ...
              {hoveredEvent.adresse && (
                <p>
                  <strong>Adresse:</strong> {hoveredEvent.adresse.numero}{" "}
                  {hoveredEvent.adresse.rue}, {hoveredEvent.adresse.codePostal}{" "}
                  {hoveredEvent.adresse.ville}
                </p>
              )}
            </div>
          )}
      ...
      <div className="relative p-4 w-full max-w-2xl max-h-full">
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <div className="p-4 md:p-5 space-y-4">
            ...
            <p className="text-base leading-relaxed text-gray-600 dark:text-gray-400">
              L'évènement aura lieu du{" "}
              {new Date(event.dateDebut).toLocaleDateString()} au{" "}
              {new Date(event.dateDebut).toLocaleDateString()}.
            </p>
            <p className="text-base leading-relaxed text-gray-600 dark:text-gray-400">
              Le coût d'inscription à cet évènement est de {event.prixNormal} €
              pour les adhérents et {event.prixAdherent} € sinon.
            </p>
            <p
              className="text-base leading-relaxed text-gray-400 dark:text-gray-400"
              style={{ fontSize: "0.8rem" }}
            >
              Une fois payé, le montant ne pourra pas être remboursé, y compris
              en cas de désinscription, ou absence. Vous recevrez un mail
              confirmant votre participation, après payement.
            </p>
          </div>
          <div className="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
            <button
              onClick={() => handleParticiper(event.idEvenement)}
            ...
            {user != null && user.isAdmin && (
              <button
                onClick={() => handleDeleteEvent(event.idEvenement)}
                className="ms-6 text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-red-600 dark:focus:ring-red-700"
              >
                Supprimer
              </button>
            )}
            <button
              onClick={() =>
                navigate(`/evenement/${event.idEvenement}/participants`)
              }
              className=" ms-6 text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
            >
